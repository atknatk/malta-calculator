                        ...
                        <div className="rounded-xl border bg-white p-6 shadow-sm dark:bg-slate-800 dark:border-slate-700">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between rounded-lg bg-slate-50 p-3 dark:bg-slate-700">
                                    <span className="text-slate-600 dark:text-slate-300">👥 Employees</span>
                                    <span className="text-xl font-bold text-amber-600">
                                        {usage.employees.used}/{usage.employees.limit === 999999 ? '∞' : usage.employees.limit}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between rounded-lg bg-slate-50 p-3 dark:bg-slate-700">
                                    <span className="text-slate-600 dark:text-slate-300">📄 Payslips (This Month)</span>
                                    <span className="text-xl font-bold text-amber-600">
                                        {usage.payslips.used}/{usage.payslips.limit === 999999 ? '∞' : usage.payslips.limit}
                                    </span>
                                </div>
                            </div>
                            {company.plan === 'free' && (
                                        ...
                                        >
                                            <div>
                                                ...
                                                <p className="text-sm text-slate-500">
                                                    {new Date(payslip.period_year, payslip.period_month - 1).toLocaleDateString('en-US', {
                                                        month: 'long',
                                                        year: 'numeric'
                                                    })}
                                                </p>
