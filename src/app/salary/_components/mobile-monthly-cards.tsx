...
function MonthCard({
  ...
  const [bonusEditValue, setBonusEditValue] = React.useState(
    item.bonus.toString(),
  );
          ...
          >
            <div className="px-4 pb-4 pt-2 border-t border-border/30">
              <div className="grid grid-cols-2 gap-3">
                <DetailItem
                  label="Gross Total"
                  value={formatMoney(item.grossTotal)}
                />
                <DetailItem
                  label="SSC Tax"
                  value={formatMoney(item.sscTax)}
                  variant="warning"
                />
                <DetailItem
                  label="Income Tax"
                  value={formatMoney(item.incomeTax)}
                  variant="danger"
                />
                <DetailItem
                  label="Paid"
                ...
                {/* Secondary Details */}
                <DetailItem
                  label="Non-Tax Benefit"
                  value={formatMoney(item.nonTaxBenefit)}
                  small
                />
                <DetailItem
                  label="Tax Benefit"
                  value={formatMoney(item.taxBenefit)}
                  small
                />
                ...
                <DetailItem
                  label="Gov. Bonus"
                  value={formatMoney(item.governmentBonus)}
                  small
                />
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-border/30">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-medium">
                    {formatMoney(item.cumulativeIncomeBase)}
                  </span>
                </div>
              </div>
            </div>
...
export function MobileMonthlyCards({
  data,
  setData,
  onBonusChange,
}: MobileMonthlyCardsProps) {
