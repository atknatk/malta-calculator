...
import { Users, Euro, Calculator, Info, Percent, ArrowRight, Scale, ExternalLink } from "lucide-react";
        ...
        <div className="space-y-8">
            ...
            >
                ...
                <p className="text-muted-foreground max-w-2xl mx-auto">
                    Calculate the minimum salary required to sponsor family members in Malta.
                </p>
            </motion.div>
                ...
                >
                    <div className="p-6 rounded-3xl bg-gradient-to-br from-primary/5 via-background to-secondary/5 border border-border/50 space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <ToggleGroup
                                options={["family-reunification", "family-member-policy"] as const}
                                value={scheme}
                                onChange={setScheme}
                                labels={{
                    ...
                    <div className="p-6 rounded-3xl bg-gradient-to-br from-primary/10 via-primary/5 to-secondary/10 border border-primary/20 space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="p-3 rounded-xl bg-background/50 text-center">
                                <div className="text-2xl font-bold">{formatCurrency(monthlyBreakdown.monthly)}</div>
                                <div className="text-xs text-muted-foreground">per month</div>
                            </div>
                            <div className="p-3 rounded-xl bg-background/50 text-center">
                                <div className="text-2xl font-bold">{formatCurrency(monthlyBreakdown.weekly)}</div>
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                                ...
                                <span className="font-semibold">{formatCurrency(result.breakdown.base)}</span>
                            </div>
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                                <span className="text-sm text-muted-foreground">Per Member (+{result.percentagePerMember}%)</span>
                                <span className="font-semibold">{formatCurrency(result.breakdown.perMemberAddition)}</span>
                            </div>
                            <div className="flex justify-between items-center p-3 rounded-xl bg-primary/10 border border-primary/20">
                                <span className="text-sm text-muted-foreground">
                                    Additional for {familyMemberCount} member{familyMemberCount > 1 ? "s" : ""}
                                </span>
                                <span className="font-semibold text-primary">+{formatCurrency(result.breakdown.totalAddition)}</span>
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <div className="grid gap-3">
                                <div className={cn(
                                    ...
                                    scheme === "family-reunification" ? "bg-primary/5 border-primary/30" : "bg-background border-border"
                                )}>
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-medium text-sm">Family Reunification</span>
                                        <span className="text-xs text-muted-foreground">Gross</span>
                                    </div>
                                    <div className="text-2xl font-bold">
                                        {formatCurrency(comparison.familyReunification.minimumRequired)}
                                    </div>
                                </div>
                                <div className={cn(
                                    "p-4 rounded-xl border",
                                    scheme === "family-member-policy" ? "bg-primary/5 border-primary/30" : "bg-background border-border"
                                )}>
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-medium text-sm">Family Member Policy</span>
                                        <span className="text-xs text-muted-foreground">Net</span>
                                    </div>
                                    <div className="text-2xl font-bold">
                                        {formatCurrency(comparison.familyMemberPolicy.minimumRequired)}
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <div className="text-sm text-muted-foreground">
                                ...
                                <ul className="list-disc list-inside space-y-1 text-xs">
                                    <li>Average wage figures are from {WAGE_DATA.year} (NSO Malta)</li>
