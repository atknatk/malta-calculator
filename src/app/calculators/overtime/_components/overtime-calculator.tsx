...
export function OvertimeCalculator() {
    ...
    const effectiveHourlyRate = inputType === "hourly" ? hourlyRate : calculateHourlyRate(annualSalary);
        ...
        <div className="space-y-8">
            ...
            >
                ...
                <p className="text-muted-foreground max-w-2xl mx-auto">
                    Calculate your overtime pay based on Malta&apos;s employment regulations.
                </p>
            </motion.div>
                ...
                >
                    <div className="p-6 rounded-3xl bg-gradient-to-br from-primary/5 via-background to-secondary/5 border border-border/50 space-y-6">
                        ...
                        <div className="space-y-3">
                            <label className="text-sm font-medium text-foreground/70">Calculate from</label>
                            <ToggleGroup
                                options={["hourly", "annual"] as const}
                                value={inputType}
                                onChange={setInputType}
                                labels={{ hourly: "💵 Hourly Rate", annual: "📊 Annual Salary" }}
                            />
                        </div>
                        ...
                        {/* Overtime Type */}
                        <div className="space-y-3">
                            <label className="text-sm font-medium text-foreground/70">Overtime Type</label>
                            <ToggleGroup
                    ...
                    <div className="p-6 rounded-3xl bg-gradient-to-br from-primary/10 via-primary/5 to-secondary/10 border border-primary/20 space-y-6">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                                ...
                                <span className="font-semibold">{formatCurrency(result.baseHourlyRate)}/hr</span>
                            </div>
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                                <span className="text-sm text-muted-foreground">Multiplier</span>
                                <span className="font-semibold text-primary">{result.multiplier}×</span>
                            </div>
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                                <span className="text-sm text-muted-foreground">Overtime Rate</span>
                                <span className="font-semibold">{formatCurrency(result.overtimeRate)}/hr</span>
                            </div>
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                        ...
                        <div className="space-y-2">
                            {rates.map((rate) => (
                                ...
                                >
                                    ...
                                    <span className="font-medium">{rate.rate} ({rate.percent})</span>
                                </div>
                            ))}
                        </div>
                    </div>
