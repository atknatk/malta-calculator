...
export function ChildrenAllowanceCalculator() {
  ...
  const result = useMemo<ChildrensAllowanceResult>(() => {
    return calculateChildrensAllowance({
      ...
      numberOfChildren:
        numberOfChildren === "" ? 1 : Math.max(1, numberOfChildren),
    });
  }, [
    grossIncome,
    sscPaid,
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Calculate your potential Children&apos;s Allowance based on household
          income for children under 16.
        </p>
      </motion.div>
          ...
          <div className="p-6 rounded-3xl bg-gradient-to-br from-pink-500/10 via-pink-500/5 to-rose-500/10 border border-pink-500/20 space-y-6">
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground mt-1">
                {formatCurrency(result.weeklyPerChild)} per child ×{" "}
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span
                  className={cn(
                    ...
                    result.rateType === "maximum" &&
                      "bg-green-500/10 text-green-600 dark:text-green-400",
                    result.rateType === "variable" &&
                      "bg-blue-500/10 text-blue-600 dark:text-blue-400",
                    result.rateType === "minimum" &&
                      "bg-amber-500/10 text-amber-600 dark:text-amber-400",
                    result.rateType === "fixed" &&
                      "bg-gray-500/10 text-gray-600 dark:text-gray-400",
                  )}
                >
                  {result.rateType.charAt(0).toUpperCase() +
                    result.rateType.slice(1)}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="text-sm text-muted-foreground space-y-2">
                <p>
                  ...
                  The weekly rate is based on the difference between €
                  {CA_CONSTANTS.MAX_INCOME_THRESHOLD.toLocaleString()} and your
                  net income. A {CA_CONSTANTS.PERCENTAGE}% rate is applied,
                  divided by 52 weeks.
                </p>
                <p>
                  <strong className="text-foreground">Rate limits:</strong>{" "}
                  Minimum €{CA_CONSTANTS.MIN_WEEKLY_RATE}/week, Maximum €
                  {CA_CONSTANTS.MAX_WEEKLY_RATE}/week per child.
                </p>
                <p>
                  <strong className="text-foreground">Payment:</strong> Paid
                  every 13 weeks (quarterly) in advance.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20">
            <div className="flex gap-3">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                an estimate only. The actual benefit is calculated by the
                Department of Social Security following a formal application.
                Income from 2 years prior is used (e.g., 2024 income for 2026
                allowance).
              </p>
            </div>
          </div>
