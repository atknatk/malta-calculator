...
import { Home, Euro, Calculator, Info, Percent, Calendar, TrendingUp, Wallet } from "lucide-react";
...
export function MortgageCalculator() {
    ...
    const [interestRate, setInterestRate] = useState<number>(MORTGAGE_CONSTRAINTS.DEFAULT_INTEREST_RATE);
    ...
    const result = useMemo<MortgageOutput>(() => {
        return calculateMortgage({
            propertyPrice,
            depositPercent: Math.max(depositPercent, MORTGAGE_CONSTRAINTS.MIN_DEPOSIT_PERCENT),
            interestRate,
            loanTermYears,
        });
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                ...
                >
                    <div className="p-6 rounded-3xl bg-gradient-to-br from-primary/5 via-background to-secondary/5 border border-border/50 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    min="50000"
                                    value={propertyPrice}
                                    onChange={(e) => setPropertyPrice(Math.max(50000, parseInt(e.target.value) || 0))}
                                    className="w-full h-14 text-xl px-5 pr-12 rounded-xl bg-background border border-border focus:border-primary focus:ring-1 focus:ring-primary/20 focus:outline-none font-semibold transition-all duration-200 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                />
                        ...
                        <div className="space-y-3">
                            <label className="text-sm font-medium text-foreground/70 flex items-center justify-between">
                                ...
                                <span className="text-primary font-semibold">{depositPercent}% ({formatCurrency(result.depositAmount)})</span>
                            </label>
                            <input
                                type="range"
                        ...
                        <div className="space-y-3">
                            <label className="text-sm font-medium text-foreground/70 flex items-center justify-between">
                                ...
                                <span className="text-primary font-semibold">{loanTermYears} years</span>
                            </label>
                            <input
                                type="range"
                    ...
                    <div className="p-4 rounded-xl bg-muted/30 space-y-3">
                        <p className="text-sm font-medium text-foreground/70">Property Price Presets</p>
                ...
                >
                    ...
                    <div className="p-6 rounded-3xl bg-gradient-to-br from-primary/10 via-primary/5 to-secondary/10 border border-primary/20 space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                                <span className="text-sm text-muted-foreground">Deposit ({depositPercent}%)</span>
                                <span className="font-semibold text-green-600">-{formatCurrency(result.depositAmount)}</span>
                            </div>
                            <div className="flex justify-between items-center p-3 rounded-xl bg-primary/10 border border-primary/20">
                                <span className="text-sm text-muted-foreground">Loan Amount</span>
                                <span className="font-semibold text-primary">{formatCurrency(result.loanAmount)}</span>
                            </div>
                            <div className="flex justify-between items-center p-3 rounded-xl bg-background/50">
                    ...
                    <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                        <div className="flex gap-3">
                            ...
                            <div className="text-sm text-muted-foreground">
                                <p className="font-medium text-foreground mb-1">Malta Mortgage Info</p>
                                <ul className="list-disc list-inside space-y-1 text-xs">
                                    <li>Minimum deposit is <strong>10%</strong> of property value</li>
                            ...
                            <tbody>
                                {result.yearlySchedule.slice(0, 10).map((row) => (
                                    <tr key={row.year} className="border-b border-border/50">
                                        ...
                                        <td className="text-right p-2 text-amber-600">{formatCurrency(row.interestPaid)}</td>
                                        <td className="text-right p-2">{formatCurrency(row.totalPaid)}</td>
