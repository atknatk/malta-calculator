...
export interface VRTInput {
    /** Vehicle type */
    vehicleType: 'car' | 'motorcycle' | 'commercial_light' | 'commercial_heavy' | 'bus' | 'trailer';
...
function getTestFrequency(vehicleType: string, age: number): number {
    const rules = TEST_FREQUENCY[vehicleType as keyof typeof TEST_FREQUENCY] || TEST_FREQUENCY.car;
...
export function calculateVRT(input: VRTInput): VRTOutput {
    ...
    if (vehicleAge >= 10 && (vehicleType === 'car' || vehicleType === 'motorcycle')) {
