 ...
 * maximum of €27.29."
 */
export function calculateChildrensAllowance(
  input: ChildrensAllowanceInput,
): ChildrensAllowanceResult {
  const {
    grossIncome,
    sscPaid,
    rentIncome,
  ...
  if (netIncome <= CA_CONSTANTS.MIN_INCOME_THRESHOLD) {
    ...
    weeklyPerChild = Math.max(
      CA_CONSTANTS.MIN_WEEKLY_RATE,
      Math.min(CA_CONSTANTS.MAX_WEEKLY_RATE, weeklyPerChild),
    );
 ...
 */
export function getRateTypeDescription(
  rateType: ChildrensAllowanceResult["rateType"],
): string {
  switch (rateType) {
    case "maximum":
