 ...
 */
export function calculateDriversLicenseFees(
  input: DriversLicenseInput,
): DriversLicenseOutput {
  const {
    licenseType,
    age,
    category,
  ...
  if (licenseType === "international") {
    ...
    breakdown.push({
      label: `License Fee (${effectiveValidity} year${effectiveValidity > 1 ? "s" : ""})`,
      amount: licenseFee,
    });
  }
  ...
  if (
    includeTheoryTest &&
    (isFirstLicense || licenseType === "category_upgrade")
  ) {
  ...
  if (
    ...
    breakdown.push({
      label: `Practical Test Fee (${category})`,
      amount: practicalTestFee,
    });
  }
  ...
  if (
    isFirstLicense ||
    (licenseType === "renewal" && age >= 65) ||
    ["C", "C1", "CE", "D", "D1", "DE"].includes(category)
  ) {
    medicalFee = ADDITIONAL_FEES.medical;
    breakdown.push({
      label: "Medical Certificate (estimated)",
      amount: medicalFee,
    });
  }
  ...
  // Photo fee
  const photoFee =
    isFirstLicense || licenseType === "renewal" ? ADDITIONAL_FEES.photo : 0;
  if (photoFee > 0) {
  ...
  // Total cost
  const totalCost =
    licenseFee + theoryTestFee + practicalTestFee + medicalFee + photoFee;
