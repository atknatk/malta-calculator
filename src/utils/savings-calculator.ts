...
export function calculateSavings(input: SavingsInput): SavingsOutput {
  ...
  for (let year = 1; year <= years; year++) {
    ...
    const monthlyAddition =
      compoundingFrequency === "monthly" ? monthlyContribution : 0;
    const yearlyAddition =
      compoundingFrequency === "yearly" ? yearContributions : 0;
    ...
    const yearWithholdingTax =
      yearInterestGross * SAVINGS_CONSTANTS.WITHHOLDING_TAX_RATE;
