...
const NON_EU_IMPORT_DUTY_RATE = 0.1; // 10% for non-EU imports

/**
 * Calculate vehicle registration tax
 */
export function calculateVehicleRegistrationTax(
  input: VehicleRegistrationTaxInput,
): VehicleRegistrationTaxOutput {
  ...
  const ageRate =
    AGE_DEPRECIATION[Math.min(vehicleAge, 10)] || AGE_DEPRECIATION[10];
